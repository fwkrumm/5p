...
int main(int argc, char** argv) {
    ...
    if (rc == -1) {
        ...
        RETURN_WITH_CODE(
            static_cast<int>(returns::ReturnCodes::MISSING_PARAMETERS));
    }

    logging_5p::SetUpLogger(config.level);

    /*
     * Create reader and apply filter if specified; if trace level is specified,
     * enable debug logging for pcapplusplus library
     */
    pcapreader::Reader reader(config.level == common::LogLevel::TRACE_LEVEL);

    if (!reader.SetPcapFile(config.path)) {
        RETURN_WITH_CODE(
            static_cast<int>(returns::ReturnCodes::PCAP_FILE_NOT_FOUND));
    }
    ...
    while (reader.NextPackage(packet)) {
        ...
        if (dataPacket.timestamp == 0U) {
            LOG_DEBUG << "skipping timestamp 0 packet (potentially not "
    ...
    if (counter == 0U) {
        LOG_WARNING << "no packets found in pcap file. Probably there are not "
                       "packets for the "
                       "specified filter or the file is corrupt. You might "
                       "want to try to open the pcap "
                       "file (with e.g. Wireshark) and cut away the first ~5 "
                       "packets and safe this as a new file.";
    }
